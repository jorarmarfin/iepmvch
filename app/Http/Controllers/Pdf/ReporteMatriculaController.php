...
class ReporteMatriculaController extends Controller
{
    ...
    function TituloColumnas(){
        ...
        PDF::Cell(150,5,"Listado de Matricula General",0,2,'C');
        #
        PDF::SetXY(35,29);
        PDF::SetTextColor(255,0,0);
    ...
    public function reportegrado()
    {
        ...
        $y = 0;
        $i = 0;
        $matriculas = Matricula::Activas()->orderBy('idgrado')->orderBy('paterno')->orderBy('materno')->orderBy('nombres')->get();
        $grado = $matriculas->first()->grado;
        $this->TituloColumnasGrado($grado);
        foreach ($matriculas as $key => $matricula) {

            if(($i%$numMaxLineas==0 && $i!=0)||($grado<>$matricula->grado)){
                PDF::AddPage('U', 'A4');
                Reportheader();
    ...
    function TituloColumnasGrado($grado = null){
        $y=45;
        $x=30;
